...
export async function processCsvRowsForImportJob(input: {
  ...
  try {
    ...
    const importError = classifyImportError(error)

    await markImportJobFailed(input.importJobId, importError.message)
    await markImportJobRunFailed(run.id, importError.message)
    ...
    await createImportJobArtifact({
      ...
      payload: {
        ...
        retriable: importError.retriable,
...
export async function processNextPendingImportJob() {
  ...
  if (!payload.rows || payload.rows.length === 0) {
    const importError = classifyImportError("Missing input rows artifact for processing")
    await markImportJobFailed(job.id, importError.message)
    await createImportJobArtifact({
      importJobId: job.id,
...
export async function processPendingImportJobsBatch(maxJobs: number = 5) {
  const normalizedMax = Math.max(1, Math.min(20, Math.floor(maxJobs)))
  const results: Array<{ processed: boolean; jobId?: string; reason?: string }> = []
